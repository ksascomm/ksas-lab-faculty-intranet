...
function ksaslab_rss_dashboard_widget() {
	...
	if ($limit == 0) echo '<div>' . __( 'The RSS Feed is either empty or unavailable.', 'ksaslab' ) . '</div>';   // fallback message
	else foreach ($items as $item) { ?>

	<h4 style="margin-bottom: 0;">
		<a href="<?php echo $item->get_permalink(); ?>" title="<?php echo mysql2date(__('j F Y @ g:i a', 'ksaslab'), $item->get_date('Y-m-d H:i:s')); ?>" target="_blank">
...
function ksaslab_custom_dashboard_widgets() {
	wp_add_dashboard_widget('ksaslab_rss_dashboard_widget', __('Custom RSS Feed (Customize in admin.php)', 'ksaslab'), 'ksaslab_rss_dashboard_widget');
	/*
...
add_action('wp_dashboard_setup', 'ksaslab_custom_dashboard_widgets');

/************* CUSTOMIZE ADMIN *******************/
// Custom Backend Footer
function ksaslab_custom_admin_footer() {
	_e('<span id="footer-thankyou">Developed by <a href="#" target="_blank">Your Site Name</a></span>.', 'ksaslab');
}

// adding it to the admin area
add_filter('admin_footer_text', 'ksaslab_custom_admin_footer');
